...
class RicoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return [[torch.tensor(screen.embeddings) for screen in indexed_trace.trace_screens], [torch.tensor(screen.descr_emb) for screen in indexed_trace.trace_screens]]
    ...
    def load_trace(self, trace_id, trace_data_path):
        ...
        if not self.idmap.get(trace_id):
            trace_to_add = RicoTrace(self.embedder, trace_data_path, True)
    ...
    def load_all_screens(self):
        for view_hierarchy_json in os.listdir(self.location + '/' + 'view_hierarchies'):
            if view_hierarchy_json.endswith('.json') and (not view_hierarchy_json.startswith('.')):
                ...
                cur_screen = RicoScreen(self.UI_embedder, json_file_path)
...
class RicoScreen():
    ...
    """
    def __init__(self, embedder, data_path):
        self.location = data_path
        package_name, labeled_text, description = self.get_rico_info()
        self.labeled_text = labeled_text
        self.UI_embedder = embedder
        self.UI_embeddings = self.process_embeddings()
        self.package_name = package_name
        self.app_description = description
        self.descr_emb = self.embed_descript()
    ...
    def process_embeddings(self):
        ...
        for ui in self.labeled_text:
            embedding = self.UI_embedder(ui)
            embeddings.append(embedding)
        return embedding

    def embed_descript(self):
        return self.UI_embedder.embedder.text_embedder(self.app_description)
